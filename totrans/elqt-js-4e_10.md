...
但首先我们将简要讨论JavaScript中表示日期和时间值的内置方法。
...
还有一个\b标记，匹配*单词边界*，即一侧是单词字符而另一侧是非单词字符的位置。不幸的是，这些标记与\w使用相同的简单概念，因此并不可靠。
...
问题是，第一行中的 ![Image](../images/apple.jpg) 被视为两个代码单元，而 {3} 仅应用于第二个单元。类似地，点只匹配单个代码单元，而不是组成玫瑰表情的两个代码单元。
