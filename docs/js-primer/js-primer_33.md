...
同じように、在命名导入中，名称`default`对应默认导入。通过以下 way，in 命名导入中 specify `default`，You can perform default import. But please note that `default` is a reserved word, so you must use `as` syntax to specify an alias.
...
それに、命名导入和默认导入的语法 can be written simultaneously. You can connect the two grammars with a comma, as shown below.
